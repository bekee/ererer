...
<div id="page-container" class="sidebar-l sidebar-o side-scroll header-navbar-fixed">
			...
			<div class="side-header side-content">
				...
				<span>
                            <img class="img-avatar img-avatar32" src="{{asset('assets/img/avatars/avatar10.jpg')}}"
                                 alt="">
                            <span class="font-w600 push-10-l">{{auth()->user()->client->first_name}} {{auth()->user()->client->last_name}}</span>
                        </span>
			</div>
			<!-- END Side Header -->
		...
		<div id="sidebar-scroll">
			...
			<div class="sidebar-content">
				...
				<div class="side-content">
					<ul class="nav-main">
						<li>
							<a href="{{url('dashboard')}}"><i class="si si-speedometer"></i><span
						...
						<li>
							...
							<ul>
								<li>
									<a href="{{url('dashboard/provide_help')}}">PROVIDE HELP</a>
								</li>
								
								<li>
									<a href="{{url('dashboard/get_help')}}">GET HELP</a>
								</li>
								<li>
									<a href="{{url('dashboard/my_referrals')}}">MY REFERRALS</a>
								</li>
						...
						<li class="open1">
							...
							<ul>
								<li>
									<a class="active" href="{{url('dashboard/referral_bonuses')}}">REFERRAL BONUS</a>
								</li>
							</ul>
						</li>
						...
						<li>
							<a class="nav-submenu" data-toggle="nav-submenu" href="#"><i class="si si-user"></i><span
										class="sidebar-mini-hide">USER ACCOUNT</span></a>
							<ul>
								<li>
									<a href="{{url('dashboard/add_bank')}}">ADD A BANK</a>
								</li>
							</ul>
						</li>
						<li>
							...
							<ul>
								<li>
									<a href="{{url('dashboard/testimonies')}}">Testimonies</a>
								</li>
								<li>
									<a href="{{url('dashboard/topics')}}">Topics</a>
								</li>
								<li>
									<a href="{{url('dashboard/discussion')}}">Discussion</a>
								</li>
								<li>
									<a href="{{url('dashboard/support')}}">Support</a>
								</li>
							</ul>
						</li>
					</ul>
			...
			<li>
				<div class="btn-group">
					...
					<ul class="dropdown-menu dropdown-menu-right">
						...
						<li>
							<a tabindex="-1" href="{{url('dashboard/add_bank')}}">
								<i class="si si-user pull-right"></i>Add Bank
							</a>
						</li>
	...
	<main id="main-container">
		...
		<div class="content bg-image overflow-hidden"
			...
			<div class="push-50-t push-15">
				<h1 class="h2 text-white animated zoomIn">@yield('dashboard')</h1>
				<h2 class="h5 text-white-op animated zoomIn">
					Welcome {{auth()->user()->client->first_name}} {{auth()->user()->client->last_name}}</h2>
			</div>
		</div>
		<!-- END Page Header -->
			...
			<div class="row items-push text-uppercase">
				<div class="col-xs-6 col-sm-2 col-sm-offset-1">
					...
					<a class="h2 font-w300 text-primary animated flipInX"
					   href="{{url('dashboard')}}">{{env("CURRENCY_SYMBOL")}} {{number_format(auth()->user()->provideHelp->sum('amount'),2)}}</a>
				</div>
				<div class="col-xs-6 col-sm-2 col-sm-offset-1">
					...
					<a class="h2 font-w300 text-primary animated flipInX"
					   href="{{url('dashboard')}}">{{env("CURRENCY_SYMBOL")}} {{number_format(auth()->user()->getHelp->sum('amount'),2)}}</a>
				</div>
				<div class="col-xs-6 col-sm-2">
					...
					<a class="h2 font-w300 text-primary animated flipInX"
					   href="{{url('dashboard')}}">{{env("CURRENCY_SYMBOL")}}
						@if(!auth()->user()->getReferral->isEmpty())
							{{empty(auth()->user()->getReferral->UserReferralBonus) ? '0.00' : number_format(auth()->user()->getReferral->UserReferralBonus->where('status','approved')->sum('amount'),2) }}
						@else
							0.00
						@endif
					</a>
				...
				<div class="col-xs-6 col-sm-2">
					...
					<a class="h2 font-w300 text-primary animated flipInX"
					   href="{{url('dashboard')}}">{{env("CURRENCY_SYMBOL")}} {{number_format(auth()->user()->myWallet->where('state','in')->sum('amount'),2) }}</a>
				</div>
				<div class="col-xs-6 col-sm-2">
					<div class="font-w700 text-gray-darker animated fadeIn">My Wallet</div>
					<div class="text-muted animated fadeIn">
						<small><i class="si si-calendar"></i> Today</small>
					</div>
					<a class="h2 font-w300 text-primary animated flipInX"
					   href="{{url('dashboard')}}">{{env("CURRENCY_SYMBOL")}} {{number_format(auth()->user()->myWallet->sum('amount'),2) }}</a>
				</div>
			</div>
		</div>
	...
	<footer id="page-footer" class="content-mini content-mini-full font-s12 bg-gray-lighter clearfix">
		...
		<div class="pull-left">
			<a class="font-w600" href="{{url('/')}}">{{env('APP_NAME')}}</a> &copy; <span
					class="">{{date('Y')}}</span>
		</div>
	</footer>
	<!-- END Footer -->
			...
			<div class="block block-themed block-transparent">
				...
				<div class="block-content">
					<div class="row text-center">
						<div class="col-xs-6">
							<a class="block block-rounded" href="{{url('dashboard')}}">
